...
router.patch('/:username', authUser, requireLogin, async function(
  ...
  try {
    ...
    //BUG 4: let user = await User.update(req.params.username, fields);
    const canChange = ['first_name', 'last_name', 'phone', 'email'];
    for (let key in fields) {
      if (!canChange.includes(key)) {
        throw new ExpressError(`Cannot change ${key}`, 401);
      }
    }
